...
USE-CASE 1: A pdf.Indirect and its Serialize() method is invoked
for one or more output files, then its underlying direct object is
finalized.  The PDF reference suggests that a reference is sometimes
written before the information required for the object being
referenced is available.  An example from the PDF reference is using
an indirect object reference for the length of the stream before
writing the stream, presumably so you can write the stream dictionary
at a moment when the length of the stream is unknown.  The stream
length is written as a separate object after the stream is completed
when the length is known with certainty.  This particular example is
somewhat contrived.  With the size of memory in modern computers it's
difficult to imagine a scenario where a stream cannot be written to a
buffer in its entirely and written as an atomic stream object.
Nonetheless, pdf.IndirectS supports this programming style
where object references are written to a file before the objects they
refer to have been completely defined.  We do *not* support this model
with streams, however, and *do* require streams to be completed in
memory before any portion of the strean is written to a file.  A
pdf.Indirect can be written before the direct object it
references has been defined.  A pdf.Indirect obtains and
reserves an object number whenever it is written to a file, whether or
not the object being referenced has been specified.  Eventually, the
Finalize() method is called passing the object being referenced.  At
that moment, the object being referenced is written to all files to
which the pdf.Indirect was written.  If the pdf.Indirect
is subsequently added to additional files, the Finalize()ed object
should also written to those files.  This, however, requires either
...
references in memory.  Until parsing is implemented, Serialize()ing a
...
unknown at that moment, but it exists in X.  Since the object exists
statically in X, it is considered to have been finalized.  If the same
object is Serialize()'ed to file Y, then it should immediately added to
file Y.  The contensts are obtained from X.

POSSIBLE TEMPORARY BEHAVIOR: The referenced object is retained by the
pdf.Indirect so that it can be "dereferenced" or written to
additional files.  In future versions that are able to parse PDF
files, the object may be discarded from memory once written and read
back from disk if it is dererenced or written to another file.  Even
better would be a weak-reference to the object, but weak references
